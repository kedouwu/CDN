...
class CaoUser
{
	...
	public function cao_vip_downum($users_id = '', $users_type = false)
	{
		...
		$data = array("is_down" => $is_down, "today_count_num" => $total_count, "today_down_num" => $this_vip_downum, "over_down_num" => $over_down_num, "over_down_endtime" => $getTime["end"]);
		return $data;
	}
...
class RiProPay
{
	...
	public function send_order_trade_success($out_trade_no, $trade_no, $info)
	{
		...
		if ($order && $order->order_type == "charge") {
			...
			$charge_money = sprintf("%0.2f", $order->order_price * $charge_rate);
			if ($CaoUser->update_balance($charge_money)) {
				$Caolog = new Caolog();
				$new_money = $old_money + $charge_money;
