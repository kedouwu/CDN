...
add_action('plugins_loaded', function(){
	...
	class WPJAM_Share_Draft{
		...
		public function add($post_id, $time){
			if($this->links){
				$this->links	= array_filter($this->links, function($link){ return $link['time'] > time(); });
				$this->links	= $this->links ?: [];
			}

			$secret	= wp_generate_password(8, false, false);

			$this->links[$post_id]	= ['time'=>$time+time(),	'secret'=>$secret];
		...
		public function exists($post_id){
			if(!empty($this->links) && !empty($this->links[$post_id]) && $this->links[$post_id]['time'] > time()){
		...
		public static function get_fields($post_id){
			if($share_link = self::get_instance()->exists($post_id)){
				return [
					...
					'share_time'	=> ['title'=>'剩余时间',	'type'=>'view',	'value'=>($share_link['time']-time()).'秒'],
				];
			}else{
				return [
					'share_time'	=> ['title'=>'分享时长',	'type'=>'number',	'value'=>'600',	'description'=>'秒']
				];
			}	
		}
